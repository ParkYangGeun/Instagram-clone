...
exports.find = async (req, res, next) => {
  try {
    ...
    const profiles = await User.find(where, "username fullName avatar bio") // find(검색조건, 필드)
      .populate({
...
exports.findOne = async (req, res, next) => {
  try {
    ...
    // 프로필 데이터 가공
    const { username, fullName, avatar, bio, isFollowing } = _profile;
...
exports.follow = async (req, res, next) => {
  try {
    ...
    // 현재 팔로우 상태인지 확인
    const _follow = await Follow.findOne({
